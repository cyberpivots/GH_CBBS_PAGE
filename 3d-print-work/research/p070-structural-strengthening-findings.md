...
- Add rear-pod floor ribs and side-wall ledges inside the existing pod envelope.
  ...
  the review mesh.
- Prefer capsule-profile ribs, rounded route nodes, small 3D radii, and
  rounded cable-entry saddle webs over square-ended rectangular ribs where the
  resulting mesh remains watertight and print-package bounds remain accepted.
- Apply 3D radius operations only where generated STL inspection stays
  watertight; keep chamfer fallback for side-wall or saddle features where a
  full fillet would damage the review mesh.
